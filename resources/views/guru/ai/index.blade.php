<x-layouts.app title="Asisten AI" subtitle="Buat draft soal, materi, dan perangkat ajar dengan Gemini">
    ...
    <div class="grid gap-6 lg:grid-cols-5">
        ...
        {{-- Riwayat — diperbarui otomatis via polling saat ada job berjalan --}}
        <x-ui.card padding="p-0" class="h-fit lg:col-span-2"
                   x-data="riwayatAi()" x-init="mulai()">
            <div class="flex items-center justify-between border-b border-ink-100 px-6 py-4">
                ...
                <span x-cloak x-show="memuat" class="flex items-center gap-1.5 text-xs text-ink-400">
        ...
        <script>
            function riwayatAi() {
                return {
                    ...
                    mulai() {
                        // Hanya polling kalau ada badge "queued"/"processing" di daftar.
                        if (this.adaProses()) this.jadwalkan();
                    },

                    adaProses() {
                        return /queued|processing/i.test(
                            document.getElementById('riwayat-list')?.textContent ?? ''
                        );
                    },

                    jadwalkan() {
                        // Poll tiap 5 detik. Worker jalan via cron (~1 menit),
                        // jadi status berubah paling lama ~1 menit tanpa refresh manual.
                        this.timer = setTimeout(() => this.perbarui(), 5000);
                    },

                    async perbarui() {
                        this.memuat = true;
                        try {
                            const res = await fetch('{{ route('ai.status') }}', {
                                headers: { 'X-Requested-With': 'XMLHttpRequest' },
                            });
                            if (res.ok) {
                                document.getElementById('riwayat-list').innerHTML = await res.text();
                            }
                        } catch (_) { /* jaringan sesaat — coba lagi di siklus berikutnya */ }
                        this.memuat = false;

                        // Lanjut polling selama masih ada yang diproses.
                        if (this.adaProses()) this.jadwalkan();
                    },
                };
            }
